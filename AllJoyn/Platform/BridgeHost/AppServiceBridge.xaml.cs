...
namespace OpenT2T.Bridge
{
    ...
    public sealed partial class AppServiceBridge : Page
    {
        ...
        private async void LaunchBackgroundProcess_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Make sure the BridgeHostStartupTask is in your AppX folder, if not rebuild the solution
                await Windows.ApplicationModel.FullTrustProcessLauncher.LaunchFullTrustProcessForCurrentAppAsync();
            }
            catch (Exception)
            {
                MessageDialog dialog = new MessageDialog("Rebuild the solution and make sure the BridgeHostStartupTask is in your AppX folder");
