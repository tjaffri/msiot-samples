...
QStatus AllJoynAboutIcon::Initialize(_In_ alljoyn_busattachment bus, _In_ std::shared_ptr<IAdapterIcon> icon)
{
    ...
    if (!image.empty())
    {
        if (image.size() > MAX_ALLJOYN_ARRAY_SIZE)
        {
    ...
    try
    {
        ...
        std::string mimeType(icon->MimeType());
