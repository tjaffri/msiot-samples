...
namespace Bridge
{
    ...
    public:
        ...
        QStatus Initialize(_In_ std::shared_ptr<IAdapter> adapter, std::shared_ptr<DsbBridge> bridge);
        QStatus	ConnectToAllJoyn();
        QStatus Shutdown();

        AllJoynFileTransfer *GetAllJoynFileTransferInstance(_In_ alljoyn_busobject busObject);
        QStatus SetAdapterConfig(_In_ std::vector<uint8_t>& adapterConfig);
        QStatus GetAdapterConfig(_Out_ std::vector<uint8_t>& pAdapterConfig);
        QStatus SetDeviceConfig(_In_ std::wstring &tempFileName, _Out_ std::shared_ptr<DsbBridge>& finalEvent);

		bool IsConfigurationAccessSecured();
		bool IsDeviceAccessSecured();

        bool GetObjectConfigItem(_In_ std::shared_ptr<IAdapterDevice> device, _Out_ DsbObjectConfig& configItem);
        void ToFile();
    ...
    private:
        ...
        friend DsbBridge;
        bool HasBusObject(_In_ alljoyn_busobject busObject);

        // parent
        std::shared_ptr<DsbBridge> m_parent;
