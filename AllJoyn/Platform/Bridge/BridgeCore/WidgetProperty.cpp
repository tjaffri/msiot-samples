...
QStatus WidgetProperty::Get(_In_z_ const char* interfaceName, _In_z_ const char* propName, _Out_ alljoyn_msgarg val) const
{
    ...
    else if (strcmp(propName, PROPERTY_OPTPARAMS_STR) == 0)
    {
        ...
        values = alljoyn_msgarg_array_create(boost::size(keys));
        CHK_POINTER(values);
        ...
        CHK_AJSTATUS(alljoyn_msgarg_set(alljoyn_msgarg_array_element(values, HINT_KEY), ARG_UINT16_ARRY_STR, boost::size(propertyHints), propertyHints));

        // Create an array of Dictionary Entries
        dictEntries = alljoyn_msgarg_array_create(boost::size(keys));
        ...
        for (int idx = 0; idx < boost::size(keys); idx++)
        {
        ...
        CHK_AJSTATUS(alljoyn_msgarg_set(val, ARG_DICT_STR, boost::size(keys), dictEntries));
