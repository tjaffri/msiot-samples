...
namespace Bridge
{
	...
	public:
        ...
        QStatus PreFileReadAction(_Out_ std::wstring &appRelativeFileName);
		QStatus PostFileWriteAction(_In_ std::wstring &appRelativeFileName, _Out_ std::shared_ptr<DsbBridge>& finalEvent);
        QStatus PostFileReadAction(void);

	private:
		std::string m_busObjectPath;
		std::string m_srcConfigFilePath;
        std::string m_tempConfigFilePath;
