...
QStatus DeviceProperty::Initialize(std::shared_ptr<IAdapterProperty> deviceProperty, PropertyInterface *propertyInterface, std::shared_ptr<BridgeDevice> parent)
{
...
QStatus Bridge::DeviceProperty::PairAjProperties()
{
    QStatus status = ER_OK;

    vector <std::shared_ptr<IAdapterAttribute>> tempList;
...
QStatus AJ_CALL DeviceProperty::GetProperty(_In_ const void* context, _In_z_ const char* /*ifcName*/, _In_z_ const char* propName, _Out_ alljoyn_msgarg val)
{
	...
	std::shared_ptr<IAdapterAttribute> adapterAttr = nullptr;
	std::shared_ptr<IAdapterValue> adapterValue = nullptr;
    ...
    adapterStatus = deviceProperty->m_adapter->GetPropertyValue(deviceProperty->m_parent->GetAdapterDevice(), deviceProperty->m_deviceProperty, adapterValue->Name(), adapterValue, request);
...
QStatus AJ_CALL DeviceProperty::SetProperty(_In_ const void* context, _In_z_ const char* /*ifcName*/, _In_z_ const char* propName, _In_ alljoyn_msgarg val)
{
    ...
    adapterStatus = deviceProperty->m_adapter->SetPropertyValue(deviceProperty->m_parent->GetAdapterDevice(), deviceProperty->m_deviceProperty, adapterValue, request);
    if (ERROR_IO_PENDING == adapterStatus &&
        nullptr != request)
    {
        // wait for completion
...
void DeviceProperty::EmitSignalCOV(std::shared_ptr<IAdapterValue> newValue, const std::vector<alljoyn_sessionid>& sessionIds)
{
    QStatus status = ER_OK;
    alljoyn_msgarg msgArg = nullptr;
