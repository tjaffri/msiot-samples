...
QStatus PropertyInterface::Create(std::shared_ptr<IAdapter> adapter, std::shared_ptr<IAdapterProperty> adapterProperty, _In_ string &name, std::shared_ptr<BridgeDevice> device)
{
    ...
    if (DsbBridge::SingleInstance()->GetConfigManager(m_adapter)->IsDeviceAccessSecured())
    {
...
bool PropertyInterface::InterfaceMatchWithAdapterProperty(std::shared_ptr<IAdapterProperty> adapterProperty)
{
    bool retVal = false;
