...
//
//**************************************************************************************************************************************
ControlPanel::ControlPanel(_In_ std::shared_ptr<IControlPanelHandler> controlPanelHandler, _In_ std::shared_ptr<IAdapterDevice> controlledDevice)
    : m_bRegistered(false)
    , m_busObject(nullptr)
    , m_bus(nullptr)
    , m_interface(nullptr)
...
//
//**************************************************************************************************************************************
QStatus ControlPanel::Initialize(_In_ std::shared_ptr<IAdapter> adapter, _In_ alljoyn_busattachment bus, _In_z_ const wchar_t *deviceName, _In_z_ const wchar_t* panelName)
{
    ...
    narrowDeviceName = StringUtils::ToUtf8(deviceName);
    narrowPanelName = StringUtils::ToUtf8(panelName);
...
//
//**************************************************************************************************************************************
ControlPanelSimple::ControlPanelSimple(_In_ std::shared_ptr<IControlPanelHandler> controlPanelHandler, _In_ std::shared_ptr<IAdapterDevice> controlledDevice)
    : ControlPanel(controlPanelHandler, controlledDevice)
    , m_pRunEditBox(nullptr)
    , m_pOutputLabel(nullptr)
    , m_pRunButton(nullptr)
    , m_pSwitch(nullptr)
	, m_pSignalHandler(new WidgetSignalHandler(this))
{
...
//
//**************************************************************************************************************************************
QStatus ControlPanelSimple::Initialize(_In_ std::shared_ptr<IAdapter> adapter, _In_ alljoyn_busattachment bus, _In_z_ const wchar_t *deviceName, _In_z_ const wchar_t* panelName)
{
    QStatus status = ER_OK;
    std::string adapterName;
    WidgetContainer* pRoot = nullptr;

    std::shared_ptr<IControlPanelHandler> cntrlPanelHandlerShr = GetControlPanelHandler();
	IControlPanelHandlerSimple* cntrlPanelHandler = (IControlPanelHandlerSimple*)cntrlPanelHandlerShr.get();
    CHK_AJSTATUS(ControlPanel::Initialize(adapter, bus, deviceName, panelName));

    adapterName = StringUtils::ToUtf8(deviceName);

    pRoot = GetRootContainer();

    // Attach a switch to the root container widget if the Switch Value is non-null
    if (cntrlPanelHandler->SwitchValue())
    {
        std::string label = cntrlPanelHandler->SwitchLabel();
    ...
    if ((cntrlPanelHandler->OutputValue() != nullptr) && (!cntrlPanelHandler->OutputValueLabel().empty()))
    {
        std::string label = cntrlPanelHandler->OutputValueLabel();
    ...
    if (!cntrlPanelHandler->RunButtonLabel().empty())
    {
        std::string labelName;
        if (!cntrlPanelHandler->RunEntryBoxLabel().empty())
        {
            labelName = cntrlPanelHandler->RunEntryBoxLabel();
        ...
        labelName = cntrlPanelHandler->RunButtonLabel();
        // Attach a text entry box to the root container widget
    ...
    {
        QStatus hr = GetAdapter()->RegisterSignalListener(cntrlPanelHandler->ChangeOfValueSignal(), m_pSignalHandler, 0) == 0 ? ER_OK : ER_FAIL;
        if (QS_FAILED(hr))
        {
            status = ER_FAIL;
        }
...
QStatus ControlPanelSimple::RunButton()
{
    ...
    std::string runArgument = nullptr;
    try
    {
...
//
//***********************************************************************************************************************************************
ControlPanelUniversal::ControlPanelUniversal(_In_ std::shared_ptr<IControlPanelHandler> controlPanelHandler, _In_ std::shared_ptr<IAdapterDevice> controlledDevice)
    : ControlPanel(controlPanelHandler, controlledDevice)
	, m_pSignalHandler(new WidgetSignalHandler(this))
{
...
//
//***********************************************************************************************************************************************
QStatus ControlPanelUniversal::Initialize(_In_ std::shared_ptr<IAdapter> adapter, _In_ alljoyn_busattachment bus, _In_z_ const wchar_t *deviceName, _In_z_ const wchar_t* panelName)
{
    QStatus status = ER_OK;
    std::string adapterName = StringUtils::ToUtf8(deviceName);
    std::shared_ptr<IControlPanelHandler> cntrlPanelHandlerShr = GetControlPanelHandler();
    WidgetContainer* pRoot = nullptr;
    int idx = 0;

    IControlPanelHandlerUniversal* cntrlPanelHandler = (IControlPanelHandlerUniversal*)cntrlPanelHandlerShr.get();
    ...
    //if (cntrlPanelHandler->ControlledProperties() != nullptr)
    {
        ...
        for (auto sourcePropertyForControl : cntrlPanelHandler->ControlledProperties())
        {
            ...
            allJoynWidgetName = StringUtils::Format("Widget%d", idx);

            // Ask the Control Panel Handler for the label and type of the specified property
            std::string label = cntrlPanelHandler->GetLabel(sourcePropertyForControl);
            ...
            switch (type)
            {
            case Bridge::ControlType::Switch:
            {
            ...
            case Bridge::ControlType::Sensor:
            {
