...
#if defined(BOOST_WINDOWS) && WINAPI_PARTITION_APP
#   include <random>
#   include <climits>
#   include <algorithm>
#endif
...
public:
    ...
    seed_rng() BOOST_NOEXCEPT
        ...
        , random_(NULL)
    {
#if defined(BOOST_WINDOWS) && WINAPI_PARTITION_DESKTOP
...
#elif defined(BOOST_WINDOWS) && WINAPI_PARTITION_APP
            std::vector<uint16_t> tempData(10); // cannot generate bytes, hold temp data for shorts
            std::independent_bits_engine<std::default_random_engine, CHAR_BIT, uint16_t> randomBytesEngine;
            std::generate(tempData.begin(), tempData.end(), std::ref(randomBytesEngine));
            std::copy((uint8_t*)tempData.data(), ((uint8_t*)tempData.data()) + sizeof(state), state);
#else
